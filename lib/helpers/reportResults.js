...
function reportResults(marshallResults, { plain = false } = {}) {
  ...
  results.forEach((result, index) => {
    ...
    if (result.notEvaluated && result.notEvaluated.length > 0) {
      result.notEvaluated.forEach((entryArray) => {
        ...
        entryArray.forEach((entry) => {
          ...
          const wrappedMessage = wrapTextWithIndent(entry.message, maxMessageWidth, indentString)
    ...
    if (!isPackageMalicious && result.suggestions && result.suggestions.length > 0) {
      result.suggestions.forEach((suggestion) => {
        const iconRich = styleText(['cyan'], 'ℹ')
        const labelRich = styleText(['cyan', 'bold'], 'Suggested older release')
        const headline = `${suggestion.packageSpec} — published ${suggestion.ageDays} days ago`
        const detail =
          "Clears npq's package-version recency windows. Re-run npq to audit this version; compatibility is not evaluated."
        const suggestionIndent = ` ${styleText(['gray'], '│')}   `
        const headlineWidth = Math.max(
          20,
          terminalWidth - getDisplayLength(' │ ℹ Suggested older release · ') - 2
        )
        const detailWidth = Math.max(20, terminalWidth - getDisplayLength(' │   ') - 2)
        const wrappedHeadline = wrapTextWithIndent(headline, headlineWidth, suggestionIndent)
        const wrappedDetail = wrapTextWithIndent(detail, detailWidth, suggestionIndent)

        resultsForPrettyPrint += `\n ${prefixRich}`
        resultsForPrettyPrint += `\n ${prefixRich} ${iconRich} ${labelRich} ${separatorRich} ${wrappedHeadline}`
        resultsForPrettyPrint += `\n${suggestionIndent}${wrappedDetail}`

        resultsForPlainTextPrint +=
          `\n  SUGGESTION: ${suggestion.packageSpec} was published ${suggestion.ageDays} days ago ` +
          'and clears the package-version recency windows. Re-run npq to audit it; ' +
    ...
    for (const marshall of allMarshallResultsList) {
      for (const value of Object.values(marshall)) {
        ...
        if (value.suggestions && value.suggestions.length > 0) {
          value.suggestions.forEach((entry) => {
            const suggestion = { ...entry }
            delete suggestion.pkg
            const key = `${suggestion.type}:${suggestion.packageSpec}`
            if (!suggestionKeys.has(key)) {
              suggestionKeys.add(key)
              issues.suggestions.push(suggestion)
            }
          })
        }
      }
