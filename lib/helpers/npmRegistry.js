...
class NpmRegistry {
  ...
  async verifyAttestations(manifest, registryKeys) {
    ...
    // Import sigstore dynamically to handle it as optional dependency
    let sigstore
    try {
      sigstore = require('sigstore')
    } catch {
      throw new Error('sigstore module is required for attestation verification but not installed')
    }
    ...
    const response = await fetch(attestationsUrl, {
      ...this.opts,
      // disable integrity check for attestations json payload
      integrity: null
    })
