...
function findAlternativeVersion({
  ...
  if (!packageInfo || !packageInfo.versions || !packageInfo.time || !semver.valid(targetVersion)) {
    return null
  }
  ...
  const candidates = Object.keys(packageInfo.versions)
    ...
    .filter(
      ({ publishedAt, ageDays }) =>
        typeof publishedAt === 'string' && ageDays !== null && ageDays > MAX_VERSION_RECENCY_DAYS
    )
