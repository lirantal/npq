...
function buildJsonReport({
  ...
  const positionalResults = Array.isArray(marshallResults)
  const packageReports = packages.map((requested, index) => {
    const results = positionalResults ? marshallResults[index] : marshallResults[requested]
    return {
      requested,
      findings: (results || []).flatMap(normalizeMarshallGroup)
    }
  })
  const findings = packageReports.flatMap((entry) => entry.findings)
  const errors = findings.filter((item) => item.severity === 'error').length
  const warnings = findings.filter((item) => item.severity === 'warning').length
  const status = failures.length > 0 ? 'failed' : errors + warnings > 0 ? 'findings' : 'clean'
