...
const RegistryConfig = require('./helpers/registryConfig')
...
async function runJsonAudit(cliArgs, dependencies = {}) {
  ...
  const RegistryConfigClass = dependencies.RegistryConfig || RegistryConfig
  const RegistryClientClass = dependencies.RegistryClient || RegistryClient
  ...
  try {
    ...
    const registryConfig = await RegistryConfigClass.load({
      argv: cliArgs.registryConfigArgs || []
    })
    const registryClient = new RegistryClientClass(registryConfig)
