...
class BaseMarshall {
  constructor(options) {
    this.debug = options.suppressOutput ? () => {} : util.debuglog('npq')
  ...
  checkPackage(pkg, ctx) {
    return this.validate(pkg)
      ...
      .catch((err) => {
        ...
        if (err.suggestion && typeof err.suggestion === 'object') {
          this.setSuggestion({
            pkg: pkg.packageString,
            ...err.suggestion
          })
  ...
  setSuggestion(suggestion) {
    this.ctx.marshalls[this.name].suggestions.push(suggestion)
  }
