  ...
  validate(pkg) {
    return this.packageRepoUtils.getDownloadInfo(pkg.packageName).then((downloadCount) => {
      ...
      if (downloadCount < DOWNLOAD_COUNT_UPPER_THRESHOLD) {
        // Format number with thousands separator using built-in API (deterministic, US style)
        const formattedCount = Number(downloadCount).toLocaleString('en-US')
        throw new Warning(
          `Detected a relatively low download-count package: ${formattedCount} downloads last month`
        )
      }
