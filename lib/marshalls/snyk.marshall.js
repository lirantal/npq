...
class Marshall extends BaseMarshall {
  ...
  isEnabled (ctx) {
    return !!this.snykApiToken
  }

  run (ctx, task) {
    const tasks = ctx.pkgs.reduce((prevPkg, currPkg) => {
      return prevPkg.concat(this.checkPackageVulns(currPkg, ctx, task))
    }, [])

    return Promise.all(tasks)
  }

  checkPackageVulns (pkg, ctx, task) {
  ...
  getSnykVulnInfo ({ packageName, packageVersion } = {}) {
    const url = `${SNYK_API_URL}/${encodeURIComponent(packageName)}/${encodeURIComponent(packageVersion)}`
    return axios
      .get(url, {
        headers: {
