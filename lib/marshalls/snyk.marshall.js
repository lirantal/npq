  ...
  getSnykVulnInfoUnauthenticated({ packageName, packageVersion }) {
    ...
    return fetch(url)
      ...
      .then((data) => {
        ...
        // @TODO change from the unauthenticated SNYK TEST URL to osv as a fallback
        if (data && data.hasOwnProperty('totalVulns')) {
          // The anonymous JSON endpoint for security tests isn't a rich vulnerability metadata
          // report but there's a way to flag malicious packages in-case they are already known
          // by the package name:
          const isMaliciousPackage = data.totalVulns >= 1 && data.resulttitle.includes('-security')
