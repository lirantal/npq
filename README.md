...
| expired domains | Normalizes maintainer email hosts to their registrable ICANN domain, then warns when that public domain does not resolve in DNS; inconclusive DNS failures are reported as not evaluated | DNS non-resolution is a risk signal, not proof that the domain is expired or available
