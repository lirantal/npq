...
jest.mock('../lib/helpers/registryClient', () => {
  return class RegistryClientMock {
    ...
    getPackageInfo(pkg) {
      return global.fetch(`https://registry.npmjs.org/${pkg}`).then((response) => response.json())
    }
...
test('repo utils delegates package metadata and downloads to RegistryClient', async () => {
  ...
  await expect(packageRepoUtils.getDownloadInfo('test-package')).resolves.toBe(1950)
