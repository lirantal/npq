...
describe('RegistryClient', () => {
  test('provides a public default client', () => {
    expect(RegistryClient.public().registryFor('left-pad')).toBe('https://registry.npmjs.org/')
  })
  ...
  test.each([404, 405, 501])(
    ...
    async (statusCode) => {
      ...
      await expect(client.getRegistryKeys('@company/tool')).rejects.toBeInstanceOf(NotEvaluated)
    }
  )
  ...
  test('caches unavailable signing-key capability per registry', async () => {
    ...
    await expect(client.getRegistryKeys('@company/one')).rejects.toBeInstanceOf(NotEvaluated)
    await expect(client.getRegistryKeys('@company/two')).rejects.toBeInstanceOf(NotEvaluated)
  ...
  test('treats empty keys as cached unavailable capability', async () => {
    ...
    await expect(client.getRegistryKeys('one')).rejects.toBeInstanceOf(NotEvaluated)
    await expect(client.getRegistryKeys('two')).rejects.toBeInstanceOf(NotEvaluated)
    expect(fetcher.json).toHaveBeenCalledTimes(1)
  })
  ...
  test('rebases advertised attestation paths onto the scoped registry', async () => {
    ...
    await expect(client.getAttestations('@company/tool', manifest)).resolves.toEqual([
      { bundle: {} }
    ])
    expect(fetcher.json).toHaveBeenCalledWith(
  ...
  test('caches unavailable attestation capability per registry', async () => {
    ...
    await expect(client.getAttestations('@company/one', manifest)).rejects.toBeInstanceOf(
      NotEvaluated
    )
    await expect(client.getAttestations('@company/two', manifest)).rejects.toBeInstanceOf(
      NotEvaluated
    )
    expect(fetcher.json).toHaveBeenCalledTimes(1)
  })
  ...
  test('treats empty attestations as unavailable', async () => {
    ...
    await expect(client.getAttestations('@company/tool', manifest)).rejects.toBeInstanceOf(
      NotEvaluated
    )
  })
  ...
  test('never includes cause text or URL credentials in public errors', async () => {
    ...
    const fetcher = {
      json: jest
        .fn()
        .mockRejectedValue(
          new Error(`request failed for https://user:${secret}@artifactory.example.test/`)
        )
    }
