...
describe('authenticated custom registry audits', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(NpmRegistry.prototype, 'verifySignatures').mockResolvedValue({ _signatures: [{}] })
  ...
  test('reports one cached signing-key capability skip and no download requests', async () => {
    ...
    const signingKeyCalls = calls.filter(({ requestPath }) => requestPath === '-/npm/v1/keys')
  ...
  test.each([
    ...
    ['malformed attestations', '-/npm/v1/attestations/', { invalid: true }]
