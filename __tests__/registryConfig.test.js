...
describe('RegistryConfig', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(globalConfig, 'registry=https://global.example.test/npm/\n')
    fs.writeFileSync(
      userConfig,
      [
  ...
  test('uses CLI registry while preserving project scoped registries and auth', async () => {
    ...
    expect(
      config.requestOptions['//scope.example.test/artifactory/api/npm/company/:_authToken']
    ).toBe('scope-token')
  ...
  test('uses environment over project and project over user and global', async () => {
    ...
    expect(environmentConfig.registryFor('left-pad')).toBe('https://environment.example.test/npm/')
    expect(projectConfig.registryFor('left-pad')).toBe('https://project.example.test/npm/')
  })

  test('preserves registry scoped basic authentication fields', async () => {
    const config = await RegistryConfig.load({ env, cwd: project })

    expect(config.requestOptions['//basic.example.test/npm/:username']).toBe('ci-user')
    expect(config.requestOptions['//basic.example.test/npm/:_password']).toBe(
      Buffer.from('ci-password').toString('base64')
    )
  })
  ...
  test('flattens CA, proxy, TLS, and registry-scoped client certificate options', async () => {
    ...
    expect(config.requestOptions.httpsProxy).toBe('https://proxy.example.test:8443/')
    expect(config.requestOptions.strictSSL).toBe(true)
    expect(config.requestOptions['//secure.example.test/npm/:certfile']).toBe(certFile)
    expect(config.requestOptions['//secure.example.test/npm/:keyfile']).toBe(keyFile)
  })

  test('provides public npm defaults without reading config files', () => {
    const config = RegistryConfig.defaults()

    expect(config.requestOptions.registry).toBe('https://registry.npmjs.org/')
    expect(config.registryFor('left-pad')).toBe('https://registry.npmjs.org/')
  })
  ...
  test('normalizes registries without trailing slashes', () => {
    ...
    expect(config.registryFor('left-pad')).toBe('https://registry.example.test/npm/')
  })
  ...
  test('reports invalid unscoped authentication as a safe config error', async () => {
    ...
    await expect(RegistryConfig.load({ env, cwd: project })).rejects.toMatchObject({
      name: 'RegistryError',
