...
| *other* | **npq-hero** on old Node: passthrough `spawnSync` status from the real package manager. **npq** / **npq-hero**: any non-zero code the spawned package manager returns (`2`, etc.). |
...
- Use **`npq`** in **audit-only** mode when you need a **non-zero exit if marshalls report warnings or errors** (`1`), while **`0`** means the audit reported no issues in the counted summary.
